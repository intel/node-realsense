...
function tests(state, options) {
  describe('Slam Instance Test - getOccupancyMap()', function() {
    let slamInstance = null;
    afterEach(() => {
      return slamInstance.stop();
    });
    it('Should get correct data getOccupancyMap() -  ' + state, function() {
...
tests('Positive - 1', null);
tests('Positive - 2', argv);
