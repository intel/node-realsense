...
// This file is only for internal devopment use. It imports the raw .node addon
'use strict';
...
pt.createPersonTracker(trackerOptions, cameraOptions).then((personTracker) => {
  ...
  operate('pause', 5000, 'pause');
  operate('resume', 10000, 'resume');
  operate('reset', 15000, 'reset');
  operate('setPersonTrackerOptions', 20000, 'setPersonTrackerOptions', trackerOptions);
  operate('setCameraOptions', 21000, 'setCameraOptions', cameraOptions);
  operate('start', 25000, 'Start');
  operate('stop', 30000, 'Stop');
}).catch((e) => {
...
function trackingTest(person) {
  ...
  if (blob) {
    console.log('--blob: width/height', blob.width, blob.height, blob.maskData);
  }
