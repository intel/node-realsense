...
let obj;
let personTracker;
describe('PointingData3D API Test', function() {
  describe('API Existance', function() {
    before(function(done) {
      ...
      pt.createPersonTracker(personTrackerConfig).then((instance) => {
        ...
        if (person && person.gestureInfo) {
          obj = person.gestureInfo.pointingInfo.worldPointingData;
            if (!fDone) {
              fDone = true;
              done();
            }
    ...
    it('PointingData3D is exist and type is correct', function() {
      assert.equal(typeof (obj), 'object');
    });

    it('PointingData3D object has an attribute named origin, type is object', function() {
      assert.equal(typeof (obj.origin), 'object');
    });

    it('origin of PointingData3D is readonly', function() {
      assert.equal(Object.getOwnPropertyDescriptor(obj, 'origin').writable, false);
    });

    it('PointingData3D object has an attribute named direction, type is object', function() {
      assert.equal(typeof (obj.direction), 'object');
    });

    it('direction of PointingData3D is readonly', function() {
