    ...
    watch: {
        'colorEnabled': function(val) {
            ...
            transporter.sendMessage({
                type: 'control',
                command: 'color', subscribe: !!val // eslint-disable-line
            });
        } // eslint-disable-line
    },
    methods: {
...
const transporter = new SpTransport(getHostname()); // eslint-disable-line
...
let poselist = []; // eslint-disable-line
...
let rgbView = new BufferViewer(); // eslint-disable-line
...
transporter.onPTDataUpdate = (timestamp, ptData) => {
    ...
    if (ptData === undefined) {
        ...
        let ptResultList = ptData.ObjectResult;

        for (let i = 0; i < ptResultList.length; i++) {
            let ptObject = ptResultList[i];

            let dataFinal = getCurrentTime(timestamp) + ';' + ptObject.pid;
            ...
            if(ptObject.hasOwnProperty('center_mass_world')) {
                ...
                // CenterMassWorldZ+ ';';
                dataFinal += ';' + CenterMassWorldZ+ ';';

                // if( Math.abs(this.prevX-CenterMassWorldX)>0.3 ||
                // Math.abs(this.prevY-CenterMassWorldY)>0.3 ||
                // Math.abs(this.prevZ-CenterMassWorldZ)>0.3) // To Reduce print overflow
                // To Reduce print overflow
                if( Math.abs(this.prevX-CenterMassWorldX)>0.1 ||
                    Math.abs(this.prevY-CenterMassWorldY)>0.1 ||
                Math.abs(this.prevZ-CenterMassWorldZ)>0.1) {
            ...
            if(ptObject.hasOwnProperty('cumulative_total')) {
                dataFinal += ptObject.cumulativeTotal;
            }
            ...
            if(ptObject.hasOwnProperty('person_bounding_box')) {
                ...
                if(ptObject.hasOwnProperty('pid')) {
                    ctx2d.fillText('pid: ' + ptObject.pid, rectX+1, rectY+10);
                }
                if(ptObject.hasOwnProperty('distance')) {
                    ctx2d.fillText('distance: ' + ptObject.distance, rectX+1, rectY+20);
                }
            }
            ...
            if(ptObject.hasOwnProperty('person_orientation')) {
                ...
                let Confidence = ptObject.gesture_color_coordinates.confidence;
                ...
                if(headPoseResult == true) {
                    ...
                    dataFinal += ';' + 'Not Detected' + ';' + Orientation + '(' + Confidence + '%)';
                }
            }
            ...
            if(ptObject.hasOwnProperty('gesture_color_coordinates')) {
                ...
                // console.log('From - t0 XY: ' + FromX + ', ' + FromY + ', ' + ToX +
                // ', ' + ToY);
...
transporter.onColorFrame = (width, height, data) => {
    rgbView.updateBuffer(data, width, height, THREE.RGBFormat); // eslint-disable-line
