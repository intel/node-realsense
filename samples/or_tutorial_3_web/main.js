...
orModule.createObjectRecognizer().then((objectRecognizer) => {
  ...
  or.on('localization', (results) => {
    if (mode === 'localization') {
      printLocalizationResults(results);
      sendLocalizationResults(results);
      setTracking(results);
    }
  });
  or.on('tracking', (results) => {
    if (mode === 'tracking') {
      printTrackingResults(results);
      sendTrackingResults(results);
    }
  });
...
function printLocalizationResults(results) {
  const w = 18;
...
function sendLocalizationResults(results) {
  if (!connected) {
    return;
  }
  let resultArray = [];
...
function printTrackingResults(results) {
  const w = 18;
...
function sendTrackingResults(results) {
  ...
  results.forEach((trackingData) => {
    let roi = trackingData.roi;
    let resultObject = {
      label: objectsNames[i++],
      rectangle: [roi.x, roi.y, roi.width, roi.height],
    };
    resultArray.push(resultObject);
  });
  let resultToDisplay = {
